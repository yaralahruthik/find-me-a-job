...
max_pages: 1                # optional; page budget (positive integer, default 1). build auto-trims the weakest non-pinned bullets to fit it and reports each drop. Overridable per-application in an overlay; --no-trim skips it.
...
11. `max_pages` (if present) is a positive integer.

`lint` is separate and **never fails a build**: it flags duty-verb openers, number-less bullets, over-long bullets, an over-`max_pages` estimate (the same line estimate `build` auto-trims to), a missing headline/email, and the `core/RESUME-RULES.md` writing-standard checks — `pronoun` (first-person pronouns), `buzzword` (self-description fluff), `references` ("references on request"), `order` (reverse-chronology broken, exact via the structured dates), `gap` (over 6 months between roles, info-level), `bullets` (over 7 in one role, concrete framings only), and `contact` (missing phone, non-name-based email). These inform; they never gate. The over-`max_pages` flag applies to **concrete framings** only — under `--segment all` (the master view) it becomes an `info`, because the master career record is allowed to run long (and `build` never auto-trims the `all` view); `all` also warns about archived bullets with no `id` (an overlay can't resurrect them).
...
max_pages: 1                # optional; override the page budget for this application (positive integer). build auto-trims the weakest non-pinned bullets to fit; raise to 2 for a senior role. --no-trim skips it.
...
1. The overlay is a mapping. Only these keys are allowed: `version`, `entry`, `role`, `segment`, `headline`, `summary`, `date_granularity`, `max_pages`, `output`, `pin`, `drop`, `include`. A content key (`bullets`, `text`, `experience`, `projects`, `skills`) is rejected with a pointer to add the bullet to the master instead.
2. `role`/`headline` are non-empty strings; `segment` is a non-empty string or list of strings; `summary` is a string or `false`; `date_granularity` (if present) is `year` or `month`; `max_pages` (if present) is a positive integer. (An unknown `role` name surfaces at build time, exit 2.)
