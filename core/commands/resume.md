...
2. **Lint (informs, never gates).** `node scripts/resume.mjs lint --segment <seg> --json`. Read the flags back as coaching, not commands:
   ...
   - `[length]` — too long, or the whole doc likely runs past its `max_pages` budget (default 1) for this segment. `build` auto-trims the weakest non-pinned bullets to fit and reports each drop, but the cleaner fix is to curate it yourself: re-tag the weakest bullets (`archive`, or a narrower segment) or pin the keepers, never delete. (Linting `--segment all` never flags length — the master record is allowed to run long; the budget applies to the framings you ship.)
...
2. **Author `data/out/<entry-id>/tailor.yaml`.** Give bullets you want to move an `id` in the master first (edit `config/resume.yaml`, show the diff). Then write the overlay — `segment`, an optional `headline`/`summary` override, and `pin`/`drop`/`include` lists of master bullet ids. Show the YAML before writing. (A legacy `config/tailor/<entry-id>.yaml` is still read as a fallback, so existing overlays keep working.) **The overlay may only select and reorder; it can never add a bullet.** If the JD wants a claim the master can't make, that's a `data/proof-backlog.md` gap to earn — never a bullet to fabricate here.
3. **Build it.** `node scripts/resume.mjs build --for <entry-id>` (add `--pdf`). Output lands in the lead's own folder: `data/out/<entry-id>/<your-name>.{html,pdf}`, named from `config/profile.yaml` `output.resume_filename` (default `{name}`, PascalCase, no spaces) so it is send-ready with no renaming. `validate` of the overlay is implicit — a bad reference (an id not in the master, or a forbidden content key) fails the build with a clear message. **Build auto-trims to `max_pages` (default 1):** if the selection would spill over, it drops the weakest non-pinned bullets to fit and reports each one — so pin the bullets that must stay, raise `max_pages` in the overlay for a two-page senior resume, or pass `--no-trim` to render the full selection. (`output.layout: flat` reverts to the old `data/out/resume-<entry-id>.html`.)
4. **Report** the folder path and the resume filename, plus what the overlay changed (pinned/dropped/re-headlined), so the tailoring is legible and reproducible.
